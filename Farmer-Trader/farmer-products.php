...
$stmtchatcount->close();
        ...
        // Query to fetch products under review, ordered by created_at (most recent first)
$reviewQuery = "
...
$reviewStmt->bind_param("i", $user_id); // Bind the user_id parameter
$reviewStmt->execute();
$reviewResult = $reviewStmt->get_result();

// Query to fetch approved products with detailed information, ordered by effective_date (most recent first)
$approvedQuery = "
...
$approvedStmt->bind_param("i", $user_id); // Bind the user_id parameter
...
<head>
    ...
    <link rel="stylesheet" href="../css/farmer.css?v=<?php echo time(); ?>">
         <link rel="stylesheet" href="../css/products-mobile.css?v=<?php echo time(); ?>">
    ...
    <style>
        ...
        .search-bar i {
            ...
            cursor: pointer;
            color: #555; /* Optional: Adjust icon color */
        }
...
<section class="home">
    ...
    <div class="main-container">
        <div class="header-container">
            ...
            <button class="btn-create-product" onclick="window.location.href='farmer-create-product.php'">Create Product</button>
        </div>
<div class="product-container">
    <div id="noProductMessage" style="display: none; text-align: center; color: #2D4A36;">
    ...
    if ($reviewResult->num_rows > 0) {
        while ($row = $reviewResult->fetch_assoc()) {
            ...
            echo "<div class='review-product-card' data-name='{$row['product_name']}' 
    ...
    if ($approvedResult->num_rows > 0) {
        while ($row = $approvedResult->fetch_assoc()) {
            ...
            echo "<div class='product-card' data-name='{$row['product_name']}' 
...
<script>
    function searchProducts() {
        ...
        // Select all product and review product cards
        const productCards = document.querySelectorAll(".product-card");
        const reviewProductCards = document.querySelectorAll(".review-product-card");

        let noProductFound = true;  // Flag to check if no products are found

        // Filter function for product cards
        productCards.forEach(product => {
            const name = product.getAttribute("data-name").toLowerCase();
            const category = product.getAttribute("data-category").toLowerCase();
            const location = product.getAttribute("data-location").toLowerCase();

            if (name.includes(input) || category.includes(input) || location.includes(input)) {
        ...
        // Filter function for review product cards
        reviewProductCards.forEach(reviewProduct => {
            const name = reviewProduct.getAttribute("data-name").toLowerCase();
            const category = reviewProduct.getAttribute("data-category").toLowerCase();
            const price = reviewProduct.getAttribute("data-price").toLowerCase();

            if (name.includes(input) || category.includes(input) || price.includes(input)) {
                reviewProduct.style.display = "";
                noProductFound = false;
            } else {
        ...
        // Show or hide "No products found" message based on the flag
        const messageContainer = document.getElementById("noProductMessage");
        if (noProductFound) {
        ...
        if (result.isConfirmed) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    ...
                    // Optionally, remove the product row from the page (if you don't want to reload the page)
