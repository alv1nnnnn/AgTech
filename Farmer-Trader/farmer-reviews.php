...
// Set the default rating filter to all (0)
...
<head>
    ...
    <link rel="stylesheet" href="../css/reviews-mobile.css?v=<?php echo time(); ?>">
...
<section class="home">
    ...
    <div class="search-bar">
        ...
        <select id="ratingFilter" onchange="applyRatingFilter()">
            <option value="0" <?php if ($rating_filter == 0) echo 'selected'; ?>>All Ratings</option>
    ...
    <?php
        // Adjust SQL query to include the selected rating filter
        $sql = "
        ...
        // Add rating filter condition if selected
        if ($rating_filter > 0) {
            $sql .= " AND r.rating_value = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ii", $user_id, $rating_filter);
        } else {
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $user_id);
        }
    ...
    <table id="reviews-table">
        ...
        <tbody>
            ...
            if ($result->num_rows === 0) {
                echo '<tr><td colspan="5" style="text-align: center;">No reviews available for this filter.</td></tr>';
            } else {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $rating = intval($row['rating_value']); // Convert rating to integer
                    for ($i = 1; $i <= 5; $i++) {
                        if ($i <= $rating) {
                            echo '<i class="bi bi-star-fill" style="color: #FFD700;"></i>'; // Filled star
                        } else {
                            echo '<i class="bi bi-star" style="color: #FFD700;"></i>'; // Empty star
                        }
                    }

                    echo '<span>(' . $rating . '.0)</span> <!-- Display the rating value --> 
                        </td>
...
// JavaScript function to apply the rating filter
function applyRatingFilter() {
    const rating = document.getElementById('ratingFilter').value;
    window.location.href = '?rating=' + rating;
}
</script>
