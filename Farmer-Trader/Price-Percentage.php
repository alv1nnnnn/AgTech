...
<head>
    ...
    <title>AgTech - Price Percentage</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.3.0/font/bootstrap-icons.css">
    <link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>
    <link rel="stylesheet" href="../css/farmer.css?v=<?php echo time(); ?>">
    <link rel="stylesheet" href="../css/price-percentage.css?v=<?php echo time(); ?>">
    <link rel="stylesheet" href="../css/price-percentage-mobile.css?v=<?php echo time(); ?>">
...
$query = "
    WITH LatestPrices AS (
        SELECT 
            ...
            pp.current_price,
            pp.effective_date,
            ROW_NUMBER() OVER (PARTITION BY pp.product_id ORDER BY pp.effective_date DESC) AS rn
        FROM 
            productprices pp
        INNER JOIN 
    ...
    SELECT 
        ...
        lp.current_price,
        COALESCE(
            (lp.current_price - LAG(lp.current_price) OVER (PARTITION BY c.category_name ORDER BY lp.effective_date)) /
            NULLIF(LAG(lp.current_price) OVER (PARTITION BY c.category_name ORDER BY lp.effective_date), 0) * 100, 
            0
        ) AS price_change
    FROM 
        category c
    ...
    LEFT JOIN 
        userproducts up ON p.product_id = up.product_id AND up.user_id = '$userId'
    WHERE 
        up.user_id = '$userId'
    ORDER BY 
        c.category_name;
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if (in_array($categoryName, $categoriesProcessed)) {
            continue;
        }

        $categoriesProcessed[] = $categoryName;

        // Default values if no data is available
        $percentageChange = ($row['price_change'] !== null && $row['price_change'] !== 0) 
            ? round($row['price_change'], 2) 
            : '0.00'; 
        $sortIconClass = ($row['price_change'] > 0) 
            ? 'up' 
            : (($row['price_change'] < 0) ? 'down' : 'neutral');

        // Set image based on category
        $image = '../images/default.jpg'; // Default image
        switch ($categoryName) {
            case 'Swine':
                $image = '../images/cpig.jpg';
                break;
            case 'Chicken':
                $image = '../images/csiken.jpg';
                break;
            case 'Poultry':
                $image = '../images/cegg.jpg';
                break;
        }

        // Display the container for the category
        echo '
            <div class="circle-wrapper">
                <div class="circle">
                    <img src="' . $image . '" alt="' . htmlspecialchars($categoryName) . '">
                </div>
                <div class="percentage-container">
                    <div class="percentage-' . strtolower($categoryName) . '">
                        ' . $percentageChange . '% 
                        <div class="sort-icon ' . $sortIconClass . '">
                            <i class="bx bx-caret-' . 
                                ($row['price_change'] > 0 ? 'up' : ($row['price_change'] < 0 ? 'down' : 'right')) . 
                            '"></i>
                        </div>
                    </div>
                </div>
...
// Fallback for categories with no data in the result
$allCategories = ['Swine', 'Chicken', 'Poultry']; // Add all categories you expect
...
<div class="price-change-history">
    ...
    <table class="price-history-table">
        ...
        <tbody>
            <?php
            // Query to fetch product price history, using LAG to get the previous price within the same category
            $historyQuery = "
                WITH PriceHistory AS (
                    SELECT 
                        ...
                        pp.current_price,
                        pp.effective_date,
                        LAG(pp.current_price) OVER (PARTITION BY c.category_name ORDER BY pp.effective_date) AS previous_price
                    FROM 
                        productprices pp
                    JOIN 
                ...
                FROM 
                    PriceHistory
                ORDER BY 
                    category_name, product_name, effective_date DESC;
            ";
            ...
            if ($historyResult->num_rows > 0) {
                while ($historyRow = $historyResult->fetch_assoc()) {
                    $categoryName = $historyRow['category_name'];
                    $productName = $historyRow['product_name'];
                    $currentPrice = $historyRow['current_price'];
                    $previousPrice = $historyRow['previous_price'];
                    $priceChange = isset($historyRow['price_change']) ? round($historyRow['price_change'], 2) : 'N/A';

                    echo "<tr>
                            <td>" . htmlspecialchars($categoryName) . "</td>
                            <td>" . htmlspecialchars($productName) . "</td>
                            <td>₱ " . number_format($currentPrice, 2) . "</td>
                            <td>₱ " . ($previousPrice !== null ? number_format($previousPrice, 2) : 'N/A') . "</td>
                            <td>" . ($priceChange !== 'N/A' ? $priceChange . '%' : 'N/A') . "</td>
                        </tr>";
                }
            } else {
                echo "<tr><td colspan='5'>No price history available.</td></tr>";
            }
