...
.stars {
    display: inline-flex;
    gap: 4px; /* Adjust gap between stars */
}
...
<?php
// SQL query to fetch rating details, reviewer name, and product name
$sql = "
    SELECT r.rating_id, r.rater_id, r.rated_user_id, r.role, r.product_id, r.rating_value, r.comment, r.created_at,
           u.first_name, u.last_name, p.product_name
    FROM review r
    LEFT JOIN user u ON r.rater_id = u.user_id
...
<table id="reviews-table">
    ...
    <?php while ($row = $result->fetch_assoc()): 
        $ratingValue = (int)$row['rating_value']; // Convert rating to integer for comparison
    ?>
        <tr>
            ...
            <td>
                <div class="stars">
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                        <i class="bi <?php echo $i <= $ratingValue ? 'bi-star-fill' : 'bi-star'; ?>"></i>
                    <?php endfor; ?>
                </div>
                <?php echo htmlspecialchars($row['rating_value']); ?>
            </td>
